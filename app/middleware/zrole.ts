...
import type { Context, ZroleConfig } from 'egg';

module.exports = (options: ZroleConfig) => {
  return async (ctx: Context, next: () => Promise<any>) => {
    ...
    try {
      ...
      let user: string[] | string = options.getUser(ctx);
      ...
      if (options.hasOwnProperty('useSuperManage') && options.useSuperManage && formatUser.indexOf(options.useSuperManage) !== -1) {
        await next();
        return false;
      }
